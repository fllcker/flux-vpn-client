...
**[`example.json`](./example.json)** в этой же папке — цельный рабочий
пример MJ-конверта (`type: "subscriptions"`), использующий разом всё, что
описано ниже: несколько подписок на разных уровнях тарифа, вложенные
группы, несколько `ConnectionVariant` на одном сервере (VLESS TCP Reality +
XHTTP Reality), Hysteria2 с Salamander-обфускацией, `routingRules` со всеми
тремя `outboundTag` и вперемешку голыми доменами/CIDR и
`geosite:`/`geoip:`-категориями, кастомные поля и фоллбек-домены (трек 23).
Проверен раунд-трипом через `Subscription.fromJson`/`toJson` — валиден.
...
```json
{
  ...
  "fallbackDomains": ["example2.com", "example3.com"],
  "fallbackDomainsUrl": "https://raw.githubusercontent.com/.../domains.json",
  "domainTimeoutMs": 2500,
...
`fallbackDomains`/`fallbackDomainsUrl`/`domainTimeoutMs` — фоллбек-домены
на случай, если основной `url` стал недоступен (домен сгорел, DNS,
блокировка) — см. ROADMAP.md, трек 23:

- `fallbackDomains` — статический список альтернативных доменов,
  перебирается по порядку (заменяется только хост в `url`, path/query
  остаются как есть).
- `fallbackDomainsUrl` — опциональная ссылка на отдельный JSON вида
  `{"domains": ["example2.com", ...]}` (например файл на GitHub raw,
  который сервис может обновлять независимо от релизов Flux) — фетчится,
  когда статический список исчерпан; результат **заменяет**
  `fallbackDomains` целиком при следующем сохранении подписки — отдельного
  кэша для этого нет, сама подписка и есть постоянное хранилище.
- `domainTimeoutMs` — таймаут одной попытки достучаться до конкретного
  домена (текущего или очередного фоллбека), по умолчанию `2500`. Лимита
  числа попыток сверху нет — перебор естественно ограничен длиной списка.

Механизм срабатывает **только на повторных рефрешах**, не на первичном
добавлении подписки — сама фоллбек-секция приходит внутри объекта
`Subscription`, значит на самый первый (упавший) запрос её ещё неоткуда
взять. Также, если фоллбек сработал, Flux запоминает как `url` именно тот
домен, на котором запрос реально прошёл — а не то, что стоит в поле `url`
самого JSON-тела ответа (сервис может не знать/не обновлять это поле под
адрес зеркала, на котором его сейчас читают).
