...
| 13 | **Tabs** | — (не было в shadcn_ui-версии) | `settings_page.dart` (мобильная раскладка — горизонтальный ряд вкладок сверху вместо list+detail с двумя шапками; на ПК секции остаются вертикальным `_SettingsNav` слева, Tabs там не используется) | TabsList/TabsTrigger, variant default, только horizontal | ✅ (скрин юзера) | ✅ `tabs.tsx` | ✅ **в проде** (`port_ui/port_tabs.dart` → `PortTabsList<T>`) |
