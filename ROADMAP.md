...
9. Запоминание выбранного сервера между запусками (см. ниже)
...
## 9. Запоминание выбранного сервера между запусками

Сейчас `selectedServerIdProvider` (`lib/features/servers/selected_server_provider.dart`)
— чистое состояние в памяти: `build()` возвращает `null`, и все три
потребителя (`connect_panel.dart`, `connection_screen.dart`, `tray.dart`)
одинаково падают на фолбэк «первый лист дерева». После перезапуска выбор
пользователя теряется, и подключение уезжает на случайный сервер — тот, что
оказался первым в списке.

Надо: сохранять выбранный id и восстанавливать его на старте.

Куда сохранять — к настройкам (`AppSettings` + `app_settings_storage.dart`),
а не в профиль: это предпочтение конкретной машины, а не часть Magic JSON,
которым делятся. Профиль (`CoreConfig`) должен оставаться переносимым.

Главное, что нельзя забыть, — **id может исчезнуть**. Сервер удаляют из
подписки, подписку сносят целиком, дерево пересобирается при рефреше (трек 0
сохраняет состояние листов, но не гарантирует, что лист вообще останется).
Поэтому на старте id обязателен к проверке на существование в текущем дереве,
и при промахе — тот же фолбэк на первый лист, что и сейчас. Иначе получим
экран подключения, ссылающийся в пустоту.

Отдельно стоит решить (и это влияет на UX сильнее, чем сама задача): выбирать
ли сохранённый сервер, если он лежит внутри группы с `AutoSelectMarker` (трек
5). Авто-режим сам решает, к какому серверу идти, и восстановленный вручную
выбор с ним конфликтует — вероятно, если сохранён узел авто-группы, надо
восстанавливать именно маркер, а не конкретный сервер под ним.

Проверить после реализации: перезапуск с выбранным сервером из подписки,
перезапуск после удаления этого сервера из подписки, перезапуск с выбранным
авто-узлом.
