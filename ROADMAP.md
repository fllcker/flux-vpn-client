...
## 20. Обновляемые geoip/geosite вместо вшитых в сборку — не начато

**Текущее состояние:** оба файла (`geoip.dat`/`geosite.dat`) сейчас целиком
...
**План (черновой, не проработан):**

1. Настройка "Обновить geoip/geosite" — качает свежие `.dat` по
   дефолтному URL (тот же источник, что и сейчас в `fetch_xray.ps1`,
   только уже во время работы приложения, а не на этапе сборки) в
   `ensureFluxLogDirectory()`-подобный каталог пользовательских данных
   (не `assets/`, тот доступен только для чтения после установки/APK).
2. Настройки "Свой geoip URL" / "Свой geosite URL" — произвольная ссылка
   на `.dat`-файл, скачивается и кладётся туда же, вместо дефолтного.
   Нужна валидация формата перед подменой рабочего файла (иначе можно
   сломать себе роутинг битым файлом без отката) — стоит хранить
   предыдущую рабочую копию и откатываться при ошибке загрузки/парсинга.
3. И Windows-движок (`xray_config_mapper.dart`/пути в
   `xray_engine_windows.dart`), и Android (`xray.location.asset` в
   `FluxVpnService`) сейчас жёстко считают, что файлы лежат рядом с
   бинарником/в `filesDir` из ассетов — оба места резолва пути нужно
   переключить на новый (обновляемый) каталог, если он существует, иначе
   откатываться на вшитую сборочную копию как есть сейчас (не оставлять
   пользователя без рабочих geoip/geosite, если он никогда не открывал
   эту настройку).
