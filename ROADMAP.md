...
## 12. Документация Magic JSON конфига в `docs/` — сделано

`docs/magic_json.md` — контракт для внешнего сервиса подписки. Ключевой
вывод, который не был очевиден из плана заранее: **генерировать сам Magic
JSON не нужно** — у Flux нет способа импортировать MJ напрямую по URL
(`profile_storage.dart` читает/пишет только локальный
`%AppData%\flux\profile.json`, снаружи недоступный). Реальная точка
интеграции — URL подписки, который Flux воспринимает как xray-json/
base64-список ссылок (уже поддерживаемые форматы, `xray_subscription_
parser.dart`/`base64_subscription_parser.dart`), плюс набор HTTP-заголовков.
Документ поэтому построен в порядке приоритета:

1. **§1 — что отдавать по URL подписки**: тело (xray-json — рекомендуется,
   единственный формат с поддержкой `routing`; или base64-список ссылок —
   проще, но без `routing`/кастомных полей) с разобранным примером каждого
   поля, которое понимает парсер (`vnext.address/port`, `realitySettings`,
   `hysteriaSettings.obfs` и т.д.); таблица HTTP-заголовков
   (`Subscription-Userinfo`, `Announce`, новый `Profile-Custom-Fields` из
   трека 14) с точным форматом и примером полного набора заголовков; как
   работает сопоставление серверов между рефрешами (по адресу:порту).
2. **§2** — как работает автогруппировка по `remarks` (используется, если
   сервис хочет читаемые вложенные группы без ручной структуры).
3. **§3** — честно зафиксировано текущее ограничение: несколько вариантов
   подключения одного физического сервера (TCP Reality / XHTTP Reality /
   Hysteria2 на одной карточке, как видно на скриншотах в разговоре) сейчас
   **не** собираются в один `ServerLeaf` при импорте из подписки — каждый
   outbound становится отдельным листом; объединение происходит только
   задним числом при мерже по совпадению адрес:порт. Если сервису это
   нужно явно — отдельная задача сверху, не описывать как работающее.
4. **§4** — сама схема MJ (`CoreConfig`/`Subscription`/`ProxyNode`/
   `ServerConfig`/`RoutingRule`) как справочная информация: что происходит
   с присланной подпиской внутри клиента.
5. **§5** — обратная совместимость: `schemaVersion` (сейчас `1`, миграций
   нет — несовпадение версии полностью обнуляет профиль), явно указано, что
   это ограничение касается гипотетического прямого импорта MJ, а не самого
   формата подписки (xray-json им не связан).

Синхронизировано с треком 14 — `Profile-Custom-Fields` в доке описан сразу
с финальным форматом (`base64:<base64 JSON-объекта>`), а не как открытый
вопрос.
...
- Остальные экраны (`SettingsPage`, диалоги — `PortDialog`, роутинг,
  импорт подписки) не адаптированы под узкую ширину вообще, только
  `ConnectionScreen`.
