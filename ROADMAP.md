...
- **Баг: фон под диалогами всё ещё 100% чёрный** (см. ниже, делаем первым)
- **Баг: ширина верхнего/нижнего блока в правой панели** (см. ниже, делаем
  вторым)

0. **Мерж состояния при рефреше подписки** (первоочередной фикс, без него
   треки 2/3/6 будут слетать при каждом обновлении подписки)
1. Hysteria2
2. Скрытие серверов
...
## Баг: фон под диалогами всё ещё полностью затемняется

Уже пробовал фиксить (снижал `dialogBarrierColor` в `dialog_style.dart`
последовательно с `0x8A000000` → `0x33000000` → `0x14000000`) — по словам
пользователя визуально ничего не изменилось, затемнение всё ещё на 100%,
хотя и происходит плавно (значит анимация `FadeTransition` в
`ShadDialogRoute` точно отрабатывает — вопрос именно в целевой
непрозрачности, которой она достигает).

То, что уже три последовательных уменьшения значения не дали вообще
никакого видимого эффекта — подозрительно само по себе: возможно, дело не
в значении `barrierColor`, а в том, что:
- пересобранный `.exe` не отражал актуальный код в момент проверки (в этой
  сессии уже было минимум одно совпадение "забыл убить старый процесс перед
  пересборкой" — см. историю с single-instance портом);
- есть второй слой затемнения помимo барьера роута — сам `ShadDialog`/тема
  (`primaryDialogTheme`/`alertDialogTheme` в shadcn_ui) может рисовать
  собственный scrim, не связанный с параметром `barrierColor`, который я
  правил — нужно проверить исходники `theme/data.dart`/`components/dialog.dart`
  на этот счёт при следующей попытке.

Пользователь подтвердил: во время работы над этим пунктом не будет
взаимодействовать с приложением, чтобы не мешать скриншотить и сверять
результат — тогда автоматический скриншотинг (через AppActivate + точные
координаты окна) можно использовать спокойно.

## Баг: ширина верхнего/нижнего блока в правой панели

`connect_panel.dart` — блок с иконкой+именем+статусом сервера (верхний) и
блок Off/Proxy/TUN (нижний) под ним всё ещё не совпадают по ширине:

- **нижний блок** (переключатель) должен быть `fit-content` — ровно по
  содержимому, без лишнего пространства;
- **верхний блок** (карточка сервера) должен быть **той же ширины**, что и
  нижний, а не своей отдельной.

Вероятная причина: сейчас имя сервера в карточке зажато в
`SizedBox(width: 200, ...)` — это фиксированные 200px независимо от
реальной длины имени, а не честный fit-content. Из-за этого `IntrinsicWidth`
считает ширину карточки от искусственных 200px, а не от фактической ширины
текста — для коротких имён карточка (и весь блок, т.к. `crossAxisAlignment.stretch`
тянет оба блока к общей ширине) оказывается шире, чем нужно. Кандидат на
фикс: заменить `SizedBox(width: 200, ...)` на `ConstrainedBox(constraints:
BoxConstraints(maxWidth: 200), ...)` — `ConstrainedBox` с одним `maxWidth`
(без фиксации `minWidth`) корректно участвует в расчёте `IntrinsicWidth` и
даёт настоящий fit-content с обрезкой по эллипсису только для реально
длинных имён, в отличие от `Expanded` (ломает `IntrinsicWidth`, см. историю
исходного бага) и `SizedBox` с фиксированной шириной (не fit-content).
...
- **Рефреш подписки затирает состояние** — `refreshSubscription()`
  (`subscription_import.dart:120-141`) берёт `root: fetched.root` целиком,
  без мержа со старым деревом. Уже сегодня это сбрасывает
  `ManualVariantSelection` при каждом обновлении подписки; после реализации
  треков 2/3/6 так же будет сбрасывать `hidden`, `routingRules` и ручной
  порядок. См. трек 0.

---

## 0. Мерж состояния при рефреше подписки

**Первоочередной фикс** — без него скрытие/роутинг/ручной порядок не
переживают ни одного обновления подписки.

1. Новая функция `merge_subscription_tree.dart`:
   `ProxyNode mergeSubscriptionTree(ProxyNode oldRoot, ProxyNode newRoot)` —
   рекурсивно обходит новое дерево, для каждого `ServerLeaf` ищет
   соответствие в старом дереве **по `VlessConfig.address`/
   `Hysteria2Config.address`** первого варианта (та же логика идентичности,
   что уже использует `import_to_proxy_nodes.dart` при группировке
   вариантов). Если совпадение найдено — переносит на новый лист:
   `hidden`, `routingRules` (трек 3), `selection` (если выбранный
   `variantId` всё ещё существует среди новых вариантов, иначе — сброс на
   авто/первый).
2. Порядок (drag-and-drop, трек 6) — сложнее, т.к. это позиция в дереве, а
   не поле листа. Подход: после мержа per-leaf состояния прогнать новый
   список `children` через сортировку по позиции, которую эти же id имели
   в **старом** дереве (сохранённый порядок), новые/незнакомые узлы
   (появились в подписке впервые) — в конец списка на тех местах, где были
   в свежих данных.
3. Сервер, который пропал из свежих данных подписки — его сохранённое
   состояние (`hidden` и т.д.) просто отбрасывается вместе с самим узлом,
   ничего специально сохранять не нужно.
4. `subscription_import.dart` `refreshSubscription()` — заменить
   `root: fetched.root` на `root: mergeSubscriptionTree(subscription.root, fetched.root)`.
5. Юнит-тесты по аналогии с `group_leaves_by_name_test.dart` — мерж с
   добавленным/удалённым/переименованным сервером, со сменой набора
   вариантов у существующего сервера.
